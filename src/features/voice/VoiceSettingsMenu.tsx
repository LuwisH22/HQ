    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="w-72 p-3">
        <fieldset className="border-0 p-0" disabled={!canSpeak}>
          ...
          <DropdownMenuRadioGroup
            value={inputMode}
            onValueChange={(value) => void voiceService.setInputMode(value as InputMode)}
          >
            {MODES.map((mode) => (
        ...
        <fieldset className="border-0 p-0" disabled={!canSpeak}>
          ...
          <Toggle
            ...
            onChange={(next) => void voiceService.setAudioProcessing({ echoCancellation: next })}
          />
          <Toggle
            label="Noise suppression"
            hint="Fans, keyboards, the street outside."
            checked={processing.noiseSuppression}
            onChange={(next) => void voiceService.setAudioProcessing({ noiseSuppression: next })}
          />
          <Toggle
            label="Automatic gain"
            hint="Evens out how close you sit to the microphone."
            checked={processing.autoGainControl}
            onChange={(next) => void voiceService.setAudioProcessing({ autoGainControl: next })}
          />
